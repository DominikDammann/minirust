...
/// The catch function is executed and writes to `x`.
#[test]
fn try_fn_unwinds() {
    let mut p = ProgramBuilder::new();
...
/// 	z = z + 11;
/// 	*data_ptr = z;
/// 	catch_unwind(try_fn, data_ptr, innter_catch_fn);
/// 	return;
/// }
///
/// fn main () {
...
/// 	// check if x and ret have the expected value
/// 	assume (x == 42);
/// 	assume (ret == 1);
/// }
/// ```
...
/// In this test a value, which is not a raw pointer, is used as data pointer in catch_unwind, which results in an ill-formed program.
#[test]
fn wrong_data_ptr() {
    let mut p = ProgramBuilder::new();
